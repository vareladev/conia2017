...
<?php
	...
	$query = 'INSERT INTO `asistentes`
			(`nombre1`, `nombre2`, `apellido1`, `apellido2`, `telefono`, `genero`, `nivel`, `institucion`, `email`, `area`, `ponente`, `info`, `torneo`,`inscripcion`) 
			VALUES 
			(\''.$nombre1.'\',\''.$nombre2.'\',\''.$ape1.'\',\''.$ape2.'\',\''.$tel.'\','.$gen.',\''.$nivel.'\',\''.$institucion.'\',\''.$mail.'\',\''.$area.'\',\''.$ponente.'\',\''.$info.'\','.$torneo.',1);';
	echo $query;
	...
	if( MAIL_NVLP($name, $Emisor, $Receptor , $titulo, $Mensaje) ){
		if($torneo == 1){
			header("Location: inscripcion3.php?reg=1&rbk=1");	
		}else{
			header("Location: inscripcion3.php?reg=1");	
		}
	}else{
		header("Location: inscripcion3.php?error=1");
	}
